//! Memory-only wrapper around `FileEditTool`.

use super::path_guard::{ensure_memory_path_required, PathField};
use crate::agentic::tools::framework::{
    Tool, ToolRenderOptions, ToolResult, ToolUseContext, ValidationResult,
};
use crate::agentic::tools::implementations::FileEditTool;
...
impl Default for MemoryEditTool {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Tool for MemoryEditTool {
    ...
    async fn description(&self) -> BitFunResult<String> {
        Ok(
            "Edit a memory file by replacing exact text. The `file_path` must be absolute and \
             resolve inside one of the auto-memory fork's allowed memory roots; edits outside \
             those roots are refused at the tool layer. Same usage as `Edit`, including the \
             requirement to `MemoryRead` the file first."
                .to_string(),
        )
    }
