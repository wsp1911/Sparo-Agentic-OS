---
name: liveapp-dev
description: Develops, maintains, and generates BitFun Live Apps (Zero-Dialect Runtime). Use when (1) working on live_app framework code under src/crates/core/src/live_app/ or src/web-ui/src/app/scenes/apps/live-app/; or (2) generating / creating / designing a NEW Live App for the user — including any request like "做一个小应用 / 生成 Live App / 写个 BitFun 小工具". Also triggers on Live App, liveapps, bridge, zero-dialect, InitLiveApp, app.fs / app.shell / app.storage, or any work under LiveApp/Demo/ and LiveApp/Skills/.
---
...
> - **生成一个新的 Live App** → **必读** [`design-playbook.md`](design-playbook.md)，并遵循下方"生成新 Live App 必读（速查）"章节的硬约束。详细 API 见 [`api-reference.md`](api-reference.md)。
...
2. **找设计上下文**：先读 `LiveApp/Demo/` 与 `src/crates/core/src/live_app/builtin/assets/` 中**最贴近形态**的内置应用，复刻它的视觉语言（间距 / 圆角 / 卡片密度 / motif）。**从零 mock 是最后选择**。
...
### 工具型 vs 展示型
绝大多数 BitFun Live App 是**工具型**——信息密集、操作短、配色冷静，仿照 `regex-playground` / `coding-selfie` / `git-graph` 的克制感。只有用户明确要"对外展示 / 灵感型 / 作品集"时才放飞视觉。
...
Live App 运行时**只暴露下列能力**，没有任何"通用 BitFun 后端通道"。设计 / 生成新小应用前请先比对，能力不在表内的需求请走相应替代方案，**不要假设有 `app.bitfun.*` / `app.workspace.*` / `app.git.*` / `app.session.*` 之类的接口存在。**
